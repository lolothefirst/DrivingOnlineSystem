...
error_log("[DEBUG] License Success Page - User ID: " . $_SESSION['user_id']);
...
if (!preg_match('/^DL[0-9]{10,25}$/i', $transaction_id)) {
...
$stmt = $conn->prepare("
    SELECT lr.*, u.full_name, u.email, u.id_number as user_id_number
    FROM license_renewals lr
    JOIN users u ON lr.user_id = u.id
    WHERE lr.transaction_id = ? AND lr.user_id = ?
");
...
if (!$renewal) {
    ...
    if (isset($_SESSION['payment_success'])) {
        ...
        $renewal['user_id_number'] = $_SESSION['id_number'] ?? 'N/A';
...
<div class="renewal-container">
    ...
    <div class="success-message non-printable" style="background: linear-gradient(135deg, #28a745 0%, #20c997 100%); color: white; padding: 40px; border-radius: 12px; text-align: center; margin-bottom: 30px; box-shadow: 0 8px 16px rgba(40, 167, 69, 0.3);">
        ...
        <p style="margin: 0; font-size: 18px; opacity: 0.95;">Your driving license has been successfully renewed</p>
        <div style="margin-top: 20px; padding: 15px; background: rgba(255,255,255,0.2); border-radius: 8px; display: inline-block;">
            <div style="font-size: 14px; opacity: 0.9;">License Number</div>
            <div style="font-size: 18px; font-weight: bold; margin-top: 5px;"><?php echo htmlspecialchars($renewal['license_number']); ?></div>
        </div>
    </div>
    ...
    <div class="receipt-box" style="background: white; padding: 30px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
        <div class="receipt-header" style="text-align: center; margin-bottom: 30px; padding-bottom: 20px; border-bottom: 2px solid #0066cc;">
            ...
            <h3 style="margin: 10px 0 5px; font-size: 20px;">Driving License Renewal Receipt</h3>
            <p style="margin: 0; color: #666; font-size: 14px;">Resit Pembaharuan Lesen Memandu</p>
        </div>
        ...
        <div class="receipt-row">
            ...
            <span><?php echo htmlspecialchars($renewal['license_number']); ?></span>
        </div>
        <div class="receipt-row">
            <strong>Date:</strong>
        ...
        <div class="receipt-row">
            ...
            <span><?php echo htmlspecialchars($renewal['full_name']); ?></span>
        </div>
        <div class="receipt-row">
            <strong>IC Number:</strong>
            <span><?php echo htmlspecialchars($renewal['ic_number'] ?? $renewal['user_id_number'] ?? 'N/A'); ?></span>
        </div>
        <div class="receipt-row">
            <strong>License Classes:</strong>
            <span><?php echo htmlspecialchars($renewal['license_types']); ?></span>
        </div>
    ...
    <div class="non-printable" style="text-align: center; margin: 30px 0;">
        ...
        <a href="license-renewal.php" class="btn-secondary" style="width: auto; padding: 15px 50px; display: inline-block; margin-right: 10px; text-decoration: none;">Renew Again</a>
    ...
    <div class="non-printable" style="background: #fff3cd; border: 1px solid #ffc107; padding: 15px; border-radius: 8px; margin-top: 20px;">
        ...
        <ul style="margin: 10px 0 0 20px;">
            <li>Your physical license card will be mailed to your registered address within 14 working days.</li>
            <li>Please bring this receipt and your IC to any JPJ office to collect your license immediately.</li>
