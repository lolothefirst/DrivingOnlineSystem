...
$errors = [];
$material_type = $_POST['material_type'] ?? 'text';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $query = "INSERT INTO learning_materials (title, description, content, material_type, file_path, video_url, category, display_order) 
                  VALUES (:title, :description, :content, :material_type, :file_path, :video_url, :category, :display_order)";
        ...
        if ($stmt->execute()) {
            logActivity($conn, $_SESSION['user_id'], 'Create learning material', 'learning_materials', $conn->lastInsertId());
            $_SESSION['success'] = "Learning material created successfully!";
            redirect('learning-materials.php');
        } else {
            $errors[] = "Failed to create learning material";
        }
    }
}
...
<div class="admin-content">
    <div class="container">
        <h1 class="page-title">Add New Learning Material</h1>
            ...
            <form method="POST" enctype="multipart/form-data" class="form">
                ...
                <div class="form-group">
                    ...
                    <select id="material_type" name="material_type" class="form-control" required>
                        <option value="text" <?php echo $material_type === 'text' ? 'selected' : ''; ?>>Text Content</option>
                        <option value="pdf" <?php echo $material_type === 'pdf' ? 'selected' : ''; ?>>PDF Document</option>
                    </select>
                </div>

                <div class="form-group">
                    <label for="category">Category *</label>
                    <input type="text" id="category" name="category" class="form-control" placeholder="e.g., Road Signs, Traffic Rules" value="<?php echo isset($_POST['category']) ? htmlspecialchars($_POST['category']) : ''; ?>" required>
                </div>
                ...
                <div class="form-group" id="content-group">
                    ...
                    <textarea id="content" name="content" class="form-control" rows="10"><?php echo isset($_POST['content']) ? htmlspecialchars($_POST['content']) : ''; ?></textarea>
                </div>
